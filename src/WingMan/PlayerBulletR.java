...
public class PlayerBulletR {
    ...
    PlayerBulletR(Image img, int x, int y, int player) {
        this.img = img;
        this.x = x;
        this.y = y;
        this.player = player;
    ...
    public void update() {
        if (!boom && !gone) {
            if (y <= -80 || x >= 580) { // offscreen - remove bullet
                x = y = -320;
                gone = true;
                ...
                x += 5; // top-rightward direction
                y -= 5;
            }
            
            // check collisions w/ enemy planes; destroy bullet in collision

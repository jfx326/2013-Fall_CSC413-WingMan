...
public class Player1 implements Observer {
    ...
    Player1(Image img, int x, int y) {
        ...
        for (int i = 1; i <= 9; i++) {
            hpimg.add(WingMan.r.getSprite("Resources/health0" + i + "-L.png"));
        }

        for (int i = 10; i <= 15; i++) {
            hpimg.add(WingMan.r.getSprite("Resources/health" + i + "-L.png"));
        }
    ...
    public void draw(Graphics g, ImageObserver obs) {
        ...
        if (!boom && lives > 0) { // draw plane if not exploded and has lives
            ...
            x = 120; // respawn at starting position
            y = 330;
            ready = true;
            spawnTime++;
        ...
        if (ready) {
            ...
            if (fire) {
                if (fireTime == 0) { // fires a bullet
                    WingMan.pBullet1.add(new PlayerBullet(pb, x + 17, y, 1));
                    if (powerUp) { // fires another two bullets
                        WingMan.pBulletL1.add(new PlayerBulletL(pbL, x + 10, y, 1));
                        WingMan.pBulletR1.add(new PlayerBulletR(pbR, x + 10, y, 1));
                    }
                    fireTime++;
    ...
    public void update(Observable obj, Object arg) {
        ...
        if (!boom) {
            switch (msg) {
                case "P1HitBoss": // instant death if crash into boss
                    health = 0;
                    break;
                case "P1HitEnemy": // major damage if crash into enemy
                    health -= 4;
                    WingMan.gs.playSound("Resources/snd_explosion2.wav");
                    break;
                case "P1HitBulletS": // hit small enemy bullet
                    health--;
                    WingMan.gs.playSound("Resources/snd_explosion1.wav");
                    break;
                case "P1HitBulletB": // hit big enemy bullet
                    health -= 2;
                    WingMan.gs.playSound("Resources/snd_explosion1.wav");
                    break;
            ...
            if (ready) {
                switch (msg) {
                    case "P1GotPowerUp": // has power-up
                        powerUp = true;
                        break;
                    case "P1GotHealth": // restore health
                        health = 16;
                        break;
                }
            }
